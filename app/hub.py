...
def tab_environment():
    ...
    if not gpus:
        ...
        for g in gpus:
            ...
            c1.write(h_badge(g.name, "#93C5FD"), unsafe_allow_html=True)
            c2.metric("VRAM Total (MB)", g.vram_total_mb if g.vram_total_mb is not None else "N/A")
            c3.metric("VRAM Used (MB)", g.vram_used_mb if g.vram_used_mb is not None else "N/A")
