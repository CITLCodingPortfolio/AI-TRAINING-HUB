...
def tab_environment():
    ...
    gpus = get_gpus()
    # CITL: normalize GPU records into SafeNS so Hub never crashes on missing attrs
    from types import SimpleNamespace

    class SafeNS(SimpleNamespace):
    ...
    def _to_ns(x):
        ...
        if hasattr(x, "__dict__"):
            return SafeNS(**x.__dict__)
        return SafeNS()

    gpus = [_to_ns(x) for x in (gpus or [])]
# CITL: normalize GPU dicts -> objects and guarantee expected attributes exist
    from types import SimpleNamespace
    gpus = [SimpleNamespace(**g) if isinstance(g, dict) else g for g in (gpus or [])]
    for g in gpus:
        # guarantee attributes exist (avoid AttributeError)
        for k in ('vram_total_mb','vram_used_mb','vram_free_mb','utilization_pct','name','temp_c','temperature_c'):
            if not hasattr(g, k): setattr(g, k, None)
        # if only one temp field is present, copy to the other
        if getattr(g,'temp_c',None) is None and getattr(g,'temperature_c',None) is not None: g.temp_c = g.temperature_c
        if getattr(g,'temperature_c',None) is None and getattr(g,'temp_c',None) is not None: g.temperature_c = g.temp_c
    ...
    if not gpus:
        ...
        for g in gpus:
            ...
            c4.metric("Temp (Â°C)", getattr(g, "temp_c", getattr(g, "temperature_c", None)) if getattr(g, "temp_c", getattr(g, "temperature_c", None)) is not None else "N/A")
